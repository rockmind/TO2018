		...
		try {
			...
			for (String[] dataLine : data) {
				...
				case BOOKS:
					...
					item = (ItemBook) new ItemBook(name, category, price, quantity,pageNumber,hardcover);
					break;
				case ELECTRONICS:
					...
					boolean mobile = Boolean.parseBoolean(reader.getValue(
							dataLine, "Mobilny"));
					item = (ItemElectronics) new ItemElectronics(name, category, price, quantity, warranty,mobile);
					break;
				case FOOD:
					String sExpirationDate;
					Date expirationDate;
					try {
						sExpirationDate = reader.getValue(dataLine, "Data ważności");
						expirationDate = new SimpleDateFormat("yyyy-MM-dd").parse(sExpirationDate);
					} catch (Exception e) {
						expirationDate = new Date();
					}
					item = (ItemFood) new ItemFood(name, category, price, quantity, expirationDate);
					break;
				case MUSIC:
					MusicGenre genre;
					try{
						genre = MusicGenre.valueOf(reader.getValue(dataLine, "Gatunek"));
					}
					catch (Exception e) {
